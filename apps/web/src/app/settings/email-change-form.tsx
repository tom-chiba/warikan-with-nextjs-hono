...
export function EmailChangeForm({ currentEmail }: { currentEmail: string }) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    await run(async () => {
      ...
      // ここでは「確認メールを送った」ことだけを示す。変更は別ナビゲーション（新アドレスのリンク
      //（/verify-email 着地）踏破）で確定するため、この時点ではメンバー一覧（["members"] キャッシュ）
      // を無効化しない（変わっていない値で更新しても無意味）。変更確定後に各画面が再マウント・
      // 再取得した時点で新メールに揃う。changeEmail の確認メール方式への正式対応
